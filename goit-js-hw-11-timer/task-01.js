...
// Плагин ожидает следующую HTML-разметку(-уже вставлена в хтмл-) и показывает четыре цифры: дни, часы, минуты и секунды в формате XX:XX:XX:XX.
...
class CountdownTimer {
  constructor({ selector, targetDate }) {
    ...
    setInterval(() => {
      ...
      this.sequentTime = targetDate - currentTime;
      if (this.sequentTime > 0) {
        updateClockTable(this.sequentTime);
      } else {
        this.sequentTime = 0;
        updateClockTable(this.sequentTime);
      }
...
function updateClockTable(time) {
  const days = Math.floor(time / (1000 * 60 * 60 * 24));
  const hours = pad(
...
new CountdownTimer({
  selector: '#timer-1',
  targetDate: new Date('Jan 7, 2020'),
});
